...
const createOrder = async (req, res, next) => {
  try {
    ...
    payload = {
      ...payload,
      userId: payload.userId ? payload.userId : req.session.userId,
    };
    // Check order max
    ...
    if (orderNotApprove.length > 0)
      return res
        .status(403)
        .json({ message: "Previous order must be resolved" });

    const order = await Order.create(payload, {
      where: {
        userId: req.session.userId,
        $or: [{ adminId: req.session.adminId }],
      },
    });
    return res
      .status(200)
      .json({ data: order, message: "Success Create Order" });
  } catch (error) {
    next(error);
  }
};
...
const createOrderAdminOnly = async (req, res, next) => {
  try {
    let payload = req.body;
    payload = {
      ...payload,
    ...
    const order = await Order.findAll({
      ...
      include: [
        ...
        },
        {
          model: Invoice,
          as: "invoice",
          attributes: ["id", "payment_status", "totalPrice"],
        },
        {
          model: Sparepart,
          as: "sparepart",
...
// get one order admin
const getOneOrderAdmin = async (req, res, next) => {
  try {
    const { id } = req.params;
    const order = await Order.findOne({
      where: { id },
...
const destroyOrderUser = async (req, res) => {
  ...
  if (order.status !== "pending")
    return res
      .status(401)
      .json({ message: "can not delete order, pelase call admin" });
